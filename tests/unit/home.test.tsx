...
describe("home workspace", () => {
  it("loads real profiles and their runtime before showing state, then supports listbox arrows", async () => {
    ...
    const profileList = await screen.findByRole("listbox", { name: "Profile overview" });
    const first = within(profileList).getByRole("option", { name: /Fabric profile/ });
    const second = within(profileList).getByRole("option", { name: /Vanilla profile/ });
  ...
  it("shows a retryable error instead of pretending that a failed profile request is empty", async () => {
    ...
    const profileList = await screen.findByRole("listbox", { name: "Profile overview" });
    expect(within(profileList).getByRole("option", { name: /Fabric profile/ })).toBeVisible();
  ...
  it("explains the account requirement and keeps launch disabled", async () => {
    ...
    expect(screen.getByRole("button", { name: "Use active Microsoft account" })).toBeVisible();
    await userEvent.click(screen.getByRole("button", { name: "Use active Microsoft account" }));
    await waitFor(() => expect(mocks.assignProfileAccount).toHaveBeenCalledWith("profile-one", "account-one"));
    expect(mocks.launch).not.toHaveBeenCalled();
  });

  it("keeps unconfigured NeoForge disabled and hides the unavailable SNine component control", async () => {
    ...
    await screen.findByText("Not configured");
    expect(await screen.findByRole("option", { name: "NeoForge" })).toBeDisabled();
    expect(screen.queryByRole("switch", { name: /Use SNine component/ })).not.toBeInTheDocument();
    expect(screen.queryByText(/No trusted component provider is configured/)).not.toBeInTheDocument();
