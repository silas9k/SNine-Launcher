...
export function CosmeticsPage() {
  ...
  const ownershipUnavailable = !snapshot.ok || !snapshot.online;
  const localPreview = snapshotFallback || ownershipUnavailable;
  const previewAsset = localPreview;
      ...
      <section className="snine-prism-cosmetics-grid" aria-label="Equipped cosmetics">
        {previewAsset ? (
          <div className="snine-prism-cosmetics-empty" role="status">
            ...
            <strong>{t("cosmetics.ownershipUnavailable")}</strong>
            <span>{t("cosmetics.ownershipDescription")}</span>
          </div>
        ) : null}
        {loading ? (
