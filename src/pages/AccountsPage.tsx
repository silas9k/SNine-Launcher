...
import { Check, ExternalLink, LogIn, RefreshCcw, ShieldCheck, Trash2 } from "lucide-react";
...
import { invalidateMinecraftSkin } from "../lib/minecraftSkinCache";
...
export function AccountsPage() {
  ...
  const refreshAccount = async (accountId: string) => {
    ...
    try {
      ...
      invalidateMinecraftSkin(accountId);
      await reload();
    } catch (error) {
      report(error);
    } finally {
  ...
  const removeAccount = async () => {
    ...
    try {
      ...
      invalidateMinecraftSkin(removeTarget.id);
            ...
            return (
              <Card className="account-card" role="listitem" key={account.id}>
                <div className="account-card__identity">
                  <MinecraftAvatar accountId={account.id} username={account.username} className="account-card__avatar" />
