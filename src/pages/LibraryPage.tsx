...
export function LibraryPage() {
  ...
  const openRuntimeSetup = useShellStore((state) => state.openRuntimeSetup);
  ...
  const saveEditor = async () => {
    ...
    try {
      const profile = editor.mode === "create"
        ? await profileCommands.create(editor.name)
        : await profileCommands.duplicate(editor.source.id, editor.name);
      setEditor(null);
      setLifecycle("active");
      await reload();
      selectProfile(profile.id);
      openRuntimeSetup(profile.id);
    } catch (error) {
      report(error);
    } finally {
      setBusy(null);
        ...
        <div className="profile-grid" role="list" aria-label={t("library.profileList") }>
          {visibleProfiles.map((profile) => (
            <Card className="profile-card" role="listitem" key={profile.id}>
              <BrandMark className="profile-card__icon" aria-hidden="true" />
              <header>
