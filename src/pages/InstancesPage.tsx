...
export function InstancesPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [workspace, nextAuth] = await Promise.all([
        instanceCommands.workspace(),
        authCommands.snapshot().catch(() => null),
      ]);
      const activeProfiles = workspace.entries.map((entry) => entry.profile);
      ...
      setStatuses(Object.fromEntries(workspace.entries.map((entry) => [entry.profile.id, entry.runtime])) as Record<string, Phase5RuntimeStatus>);
      setSettings(Object.fromEntries(workspace.entries.map((entry) => [entry.profile.id, entry.settings])));
  ...
  useEffect(() => {
    if (!launches.some((item) => ["preparing", "checking-files", "downloading", "starting", "running", "stopping"].includes(item.state))) return;
    const timer = window.setInterval(() => {
      void runtimeCommands.launchStatuses().then((next) => {
        setLaunches(next);
        if (!next.some((item) => ["preparing", "checking-files", "downloading", "starting", "running", "stopping"].includes(item.state))) {
          void refresh();
        }
      }).catch(() => undefined);
    }, 1_500);
  ...
  const createInstance = async () => {
    ...
    try {
      ...
      await Promise.all([
        instanceCommands.saveSettings(created.id, normalizedSettings()),
        draft.accountId ? authCommands.assignProfileAccount(created.id, draft.accountId) : Promise.resolve(),
      ]);
