...
const BADGE_ICON_FILES = {
  snine_icon: "snine_icon.png",
  snine_icon_blue: "snine_icon_blue.png",
  snine_icon_black_gradient: "snine_icon_black_gradient.png",
  snine_icon_summer: "snine_icon_summer.png",
} as const;

type LauncherBadgeIconKey = keyof typeof BADGE_ICON_FILES;

function canonicalBadgeIconKey(value: string | null | undefined, plusActive: boolean): LauncherBadgeIconKey {
  const normalized = String(value ?? "").trim().toLowerCase();
  if (!normalized || ["default", "clear", "off", "none"].includes(normalized)) {
    return plusActive ? "snine_icon_blue" : "snine_icon";
  }
  if (["normal", "snine", "snine_icon", "white", "sninewhite", "snine_white", "snine_icon_white", "s_icon_white"].includes(normalized)) {
    return "snine_icon";
  }
  if (["blue", "snineblue", "snine_blue", "snine_icon_blue", "s_icon_blue"].includes(normalized)) {
    return "snine_icon_blue";
  }
  if (["black", "snineblack", "snine_black", "snine_icon_black", "s_icon_black", "black_gradient", "blackgradient", "graphite", "snine_icon_black_gradient"].includes(normalized)) {
    return "snine_icon_black_gradient";
  }
  if (["summer", "holiday", "vacation", "beachball", "snine_icon_summer"].includes(normalized)) {
    return "snine_icon_summer";
  }
  return plusActive ? "snine_icon_blue" : "snine_icon";
}

export function launcherBadgeIconUrl(value?: string | null, plusActive = false): string {
  const key = canonicalBadgeIconKey(value, plusActive);
  // The authenticated backend/client is authoritative for unlocks. Do not turn a
  // valid selected icon back into the default just because a stale launcher
  // snapshot temporarily says that PLUS is inactive.
  return `./snine-icons/${BADGE_ICON_FILES[key]}`;
}
...
export async function pollSNineLauncherLiveState(live: LauncherLiveSync): Promise<LauncherLiveState> {
  if (!hasTauri()) {
    return { ok: false, online: false, badgeIcon: "", plusActive: false, equippedCosmetics: {}, statusMessage: "desktop_runtime_required" };
  }
