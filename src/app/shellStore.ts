...
export type ShellPage = "home" | "profiles" | "cosmetics" | "mods" | "modpacks" | "shaders" | "resourcepacks" | "servers" | "stats" | "accounts" | "skins" | "capes" | "settings";
...
interface ShellState {
  ...
  runtimeSetupProfileId: string | null;
  toasts: ToastMessage[];
  ...
  openRuntimeSetup: (profileId: string) => void;
  clearRuntimeSetup: () => void;
  saveSettings: (settings: ShellSettings, successKey?: TranslationKey | null) => Promise<boolean>;
  resetSettings: () => Promise<boolean>;
  pushToast: (tone: ToastTone, messageKey: TranslationKey, params?: Record<string, string | number>) => void;
...
export const useShellStore = create<ShellState>((set, get) => ({
  ...
  openRuntimeSetup: (profileId) => set({ page: "home", runtimeSetupProfileId: profileId, mobileNavigationOpen: false }),
  clearRuntimeSetup: () => set({ runtimeSetupProfileId: null }),
  ...
  pushToast: (tone, messageKey, params) => set((state) => ({
    toasts: [...state.toasts.slice(-5), { id: ++toastId, tone, messageKey, params }],
  })),
