...
export function RuntimePanel({ profile }: { profile: Phase4Profile | null }) {
  ...
  const localizeError = useCallback((error: unknown, fallback: TranslationKey) => {
    ...
    return t(knownKey ?? fallback, typed?.params);
  }, [t]);
  ...
  useEffect(() => {
    let current = true;
    if (!draft.minecraftVersion || draft.loaderKind !== "fabric") {
      setFabricCatalog([]);
      return () => {
        current = false;
      };
    ...
    void runtimeCommands.catalog(draft.minecraftVersion)
      .then((next) => {
        if (!current) return;
        setFabricCatalog(next.fabricVersions);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void refreshStatus(profile.id, false);
    }, 2_000);
    return () => window.clearInterval(interval);
  }, [launch?.launchId, profile, refreshStatus]);
  ...
  const installRuntime = async (event: FormEvent) => {
    ...
    if (!profile || runtimeLocked) return;
    ...
    try {
      ...
      await refreshStatus(profile.id, false);
  ...
  const repairRuntime = async () => {
    ...
    try {
      await runtimeCommands.repair(profile.id);
      await refreshStatus(profile.id, false);
  ...
  const launchRuntime = async () => {
    ...
    try {
      const nextLaunch = await runtimeCommands.launch(profile.id, memoryMb);
  ...
  const removeComponent = async () => {
    ...
    try {
      await runtimeCommands.setComponent(profile.id, { mode: "disabled" });
      await refreshStatus(profile.id, false);
      ...
      <fieldset disabled={runtimeLocked || loadState === "loading"}>
        ...
        <div className="runtime-setup__grid">
          ...
          <SelectField
            ...
            onChange={(event) => patchDraft({
              javaMajor: Number(event.currentTarget.value) as 17 | 21,
            })}
          >
    ...
    >
      ...
      {!profile ? (
        ...
        <div className="home-status__selection">
          ...
          <span>
            ...
            <small>{t("library.revision", { revision: profile.activeRevisionId.slice(-8) })}</small>
          </span>
        </div>
      )}
      ...
      {profile && loadState === "error" ? (
        <div className="runtime-inline-error" role="alert">
          ...
          <Button onClick={() => void refreshStatus(profile.id)}>
      ...
      {profile && loadState === "ready" && status ? (
        <>
          ...
          {!profile.accountId && status.installState === "installed" ? (
            <Status tone="warning" label={t("runtime.account.requiredTitle")}>
              {t("runtime.account.required")}
            </Status>
          ) : null}
