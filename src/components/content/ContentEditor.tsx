...
import { recommendedSafeDefaultsForLoader } from "../../lib/defaultModRecommendations";
...
type DetailTab = "description" | "changelog" | "versions" | "dependencies";
...
export function ContentEditor({
  ...
  const [browseVersions, setBrowseVersions] = useState<string[]>([]);
  const [browseMinecraftVersion, setBrowseMinecraftVersion] = useState("");
  const [browseLoader, setBrowseLoader] = useState<Phase6ContentSnapshot["loader"]>("fabric");
  ...
  useEffect(() => {
    if (mode !== "discover" || snapshot?.minecraftVersion) return;
    let current = true;
    void runtimeCommands.catalog().then((catalog) => {
      if (!current) return;
      const versions = catalog.minecraftVersions.map((entry) => entry.version);
      setBrowseVersions(versions);
      setBrowseMinecraftVersion((selected) => selected || versions.find((version) => /^1\.21\./.test(version)) || versions[0] || "");
    }).catch(() => {
      // Search remains safely disabled until the controlled catalog is ready.
      // No guessed compatibility values are sent to Modrinth.
    });
    return () => {
      current = false;
    };
  }, [mode, snapshot?.minecraftVersion]);

  const effectiveMinecraftVersion = snapshot?.minecraftVersion ?? browseMinecraftVersion;
  const effectiveLoader = snapshot?.loader ?? browseLoader;
  const canInstall = Boolean(snapshot?.minecraftVersion && snapshot.loader);
  ...
  const recommendedDefaults = useMemo(() => {
    if (!snapshot?.minecraftVersion || !snapshot.loader) return [];
    return recommendedSafeDefaultsForLoader(snapshot.loader);
  }, [snapshot?.loader, snapshot?.minecraftVersion]);

  const installRecommended = async (projectId: string) => {
    const detailForProject = await contentCommands.project(profileId, projectId);
    const candidateVersion = detailForProject.versions.find((version) => version.compatible)
      ?? detailForProject.versions[0];
    if (!candidateVersion) {
      throw new Error("content_recommendation_version_missing");
    }
    setDetail(detailForProject);
    setDetailTab("description");
    setVersionId(candidateVersion.versionId);
    await contentCommands.install(profileId, detailForProject.projectId, candidateVersion.versionId);
    await refreshSnapshot(profileId, false);
    setLiveMessage(t("content.operation.installed"));
  };

  const installRecommendedDefaults = async () => {
    if (!recommendedDefaults.length) return;
    for (const entry of recommendedDefaults) {
      await installRecommended(entry.projectId);
    }
  };
  ...
  const searchAt = async (offset: number) => {
    if (!effectiveMinecraftVersion || !effectiveLoader || query.trim().length < 2) return;
      ...
      <Card className="content-editor content-editor--empty">
        <EmptyState
          ...
          action={<>
            ...
            {onOpenProfiles ? <Button onClick={onOpenProfiles}><FilePlus2 aria-hidden="true" />{t("nav.profiles")}</Button> : null}
          </>}
        />
  ...
  return (
    <section className={`content-editor ${mode === "discover" ? "content-editor--browser" : ""}`} aria-labelledby="content-editor-title">
        ...
        <div className="content-editor__profile-actions">
          ...
          {mode === "discover" && kind === "modpack" ? <>
            ...
            <Button
              ...
              disabled={snapshot?.localFileCapability.state !== "available" || !snapshot?.minecraftVersion || !snapshot?.loader}
      ...
      {mode === "discover" && snapshotState === "ready" && (!snapshot?.minecraftVersion || !snapshot.loader) ? (
        <Status tone="warning" label={t("status.warning")}>
          {t("content.runtimeRequired")}
          {onOpenProfiles ? <Button onClick={onOpenProfiles}>{t("nav.profiles")}</Button> : null}
        </Status>
      ) : null}
      ...
      {snapshotState === "ready" && workspace === "discover" ? (
        <div id="content-discover-panel" role="tabpanel" className={`content-discover ${detailState === "ready" && detail ? "content-discover--detail" : ""}`}>
          <Card className="content-search-panel">
            <form onSubmit={submitSearch} className="content-search-form">
              ...
              />
              <SelectField
                label={t("content.minecraftFilter")}
                value={effectiveMinecraftVersion}
                disabled={Boolean(snapshot?.minecraftVersion)}
                onChange={(event) => setBrowseMinecraftVersion(event.currentTarget.value)}
              >
                {snapshot?.minecraftVersion ? <option value={snapshot.minecraftVersion}>{snapshot.minecraftVersion}</option> : null}
                {!snapshot?.minecraftVersion && !browseVersions.length ? <option value="">{t("runtime.catalog.loading")}</option> : null}
                {!snapshot?.minecraftVersion ? browseVersions.map((version) => <option key={version} value={version}>{version}</option>) : null}
              </SelectField>
              <SelectField
                label={t("content.loaderFilter")}
                value={effectiveLoader ?? ""}
                disabled={Boolean(snapshot?.loader)}
                onChange={(event) => setBrowseLoader(event.currentTarget.value as Phase6ContentSnapshot["loader"])}
              >
                {snapshot?.loader ? <option value={snapshot.loader}>{t(loaderKey(snapshot.loader))}</option> : null}
                {!snapshot?.loader ? <>
                  <option value="fabric">{t("runtime.loader.fabric")}</option>
                  <option value="vanilla">{t("runtime.loader.vanilla")}</option>
                  <option value="neoforge">{t("runtime.loader.neoforge")}</option>
                </> : null}
              </SelectField>
              <Button
                type="submit"
                ...
                disabled={query.trim().length < 2 || !effectiveMinecraftVersion || !effectiveLoader}
              ><Search aria-hidden="true" />{t("app.search")}</Button>
            </form>
            {!canInstall ? (
              <Status tone="warning" label={t("status.warning")}>{t("content.runtimeRequired")}</Status>
            ) : null}
            {recommendedDefaults.length ? (
              <Card className="content-recommendations">
                <header className="content-recommendations__header">
                  <div>
                    <p className="page-eyebrow">{t("content.recommendedDefaultsEyebrow")}</p>
                    <h3>{t("content.recommendedDefaults")}</h3>
                  </div>
                  <Button
                    variant="primary"
                    loading={busy === "recommended-bundle"}
                    disabled={busy !== null || !canInstall}
                    onClick={() => {
                      setBusy("recommended-bundle");
                      setErrorMessage(null);
                      void installRecommendedDefaults().catch((error) => {
                        setErrorMessage(localizeError(error));
                      }).finally(() => {
                        setBusy(null);
                      });
                  ...
                  >
                    {t("content.installSafeDefaults")}
                  </Button>
                </header>
                <ul className="content-recommendations__list">
                  {recommendedDefaults.map((entry) => (
                    <li key={entry.projectId}>
                      ...
                      <Button
                        variant="primary"
                        loading={busy === `recommended:${entry.projectId}`}
                        disabled={busy !== null || !canInstall}
                        onClick={() => {
                          setBusy(`recommended:${entry.projectId}`);
                          setErrorMessage(null);
                          void installRecommended(entry.projectId).catch((error) => {
                            setErrorMessage(localizeError(error));
                          }).finally(() => {
                            setBusy(null);
                          });
                      ...
                      >
                        {t("content.installRecommended", { title: entry.title })}
                      </Button>
                    </li>
                  ))}
                </ul>
                  ...
                  {searchResult.hits.map((hit) => (
                    <li key={hit.projectId}>
                      <button type="button" aria-pressed={detail?.projectId === hit.projectId} onClick={() => void loadDetail(hit.projectId)}>
                        ...
                        <Badge tone="neutral">{effectiveMinecraftVersion || hit.latestVersion || t("content.versionUnknown")}</Badge>
                      </button>
                    </li>
                  ))}
                </ul>
          ...
          <Card className="content-project-panel" aria-live="polite">
            ...
            {detailState === "ready" && detail ? (
              <div className="content-project-detail">
                <header className="content-project-detail__bar">
                  <button type="button" onClick={() => { setDetail(null); setDetailState("idle"); }} aria-label={t("content.detailBack")}><ChevronLeft aria-hidden="true" /></button>
                  <strong>{t("content.detailTitle")}</strong>
                </header>
                <div className="content-project-detail__top">
                  <section className="content-project-detail__identity">
                    ...
                    <div className="content-project-detail__badges">
                      {detail.categories.slice(0, 3).map((category) => <Badge key={category} tone="neutral">{category}</Badge>)}
                    </div>
                    <dl className="content-project-detail__stats">
                      <div><dt>{t("content.downloads")}</dt><dd>{formatNumber(detail.downloads)}</dd></div>
                      <div><dt>{t("content.detailFollowers")}</dt><dd>{formatNumber(detail.followers)}</dd></div>
                    </dl>
                  </section>
                  <aside className="content-project-detail__about">
                    <h4>{t("content.detailAbout")}</h4>
                    <dl>
                      <div><dt>{t("content.version")}</dt><dd>{selectedVersion?.versionNumber ?? "—"}</dd></div>
                      <div><dt>{t("content.loaderFilter")}</dt><dd>{effectiveLoader ? t(loaderKey(effectiveLoader)) : "—"}</dd></div>
                      <div><dt>{t("content.minecraftFilter")}</dt><dd>{effectiveMinecraftVersion || "—"}</dd></div>
                      <div><dt>{t("content.detailUpdated")}</dt><dd>{formatDate(detail.updatedAtUnix * 1000)}</dd></div>
                      <div><dt>{t("content.license")}</dt><dd>{detail.license}</dd></div>
                    </dl>
                    <SelectField label={t("content.version")} value={versionId} onChange={(event) => setVersionId(event.currentTarget.value)}>
                      ...
                      {detail.versions.map((version) => <option key={version.versionId} value={version.versionId} disabled={!version.compatible}>{version.versionNumber}</option>)}
                    </SelectField>
                    <Button variant="primary" loading={busy === `install:${detail.projectId}`} disabled={!canInstall || !selectedVersion?.compatible || hasBlockingConflict || busy !== null} onClick={() => void installSelected()}><Download aria-hidden="true" />{t("content.installVerified")}</Button>
                  </aside>
                </div>
                <Tabs
                  label={t("content.detailTitle")}
                  value={detailTab}
                  onChange={(value) => setDetailTab(value as DetailTab)}
                  items={[
                    { value: "description", label: t("content.detailDescription") },
                    { value: "changelog", label: t("content.detailChangelog") },
                    { value: "versions", label: t("content.detailVersions") },
                    { value: "dependencies", label: t("content.dependencies") },
                  ]}
                />
                <div className="content-project-detail__lower">
                  <section className="content-project-detail__body">
                    {detailTab === "description" ? <p>{detail.description}</p> : null}
                    {detailTab === "changelog" ? <p>{t("content.detailUpdated")}: {formatDate(detail.updatedAtUnix * 1000)}</p> : null}
                    {detailTab === "versions" ? <ul className="content-project-detail__versions">{detail.versions.map((version) => <li key={version.versionId}><button type="button" disabled={!version.compatible} onClick={() => setVersionId(version.versionId)}><strong>{version.versionNumber}</strong><small>{formatDate(version.publishedAtUnix * 1000)}</small></button></li>)}</ul> : null}
                    {detailTab === "dependencies" ? <p>{selectedVersion?.dependencies.length ? t("content.compatibilityScope") : t("content.noDependencies")}</p> : null}
                    {hasBlockingConflict ? <Status tone="warning" label={t("status.warning")}>{t("content.installNeedsResolution")}</Status> : null}
                  </section>
                  <section className="content-relations content-project-detail__dependencies" aria-labelledby="project-dependencies-title">
