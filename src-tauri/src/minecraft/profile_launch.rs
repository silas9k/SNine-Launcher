...
impl ProfileProcessManager {
    pub async fn launch(
        ...
        let mut log_file = OpenOptions::new()
            ...
            .append(true)
            .open(&log_path)
            .map_err(|_| AppError::coded("runtime_launch_log_create_failed"))?;
        let diagnostic_args = redact_launch_arguments(&args);
        write_launch_diagnostics(
            &mut log_file,
            &launch_id,
            request.profile_id,
            request.java_executable,
            &paths.instance,
            &diagnostic_args,
        )?;
        let _ = crate::logging::append(&format!(
            "[MINECRAFT START] launchId={launch_id} profileId={} java={} gameDir={}\nArguments:\n{}",
            request.profile_id,
            request.java_executable.display(),
            paths.instance.display(),
            diagnostic_args.join("\n"),
        ));
        let error_file = log_file
            .try_clone()
            .map_err(|_| AppError::coded("runtime_launch_log_clone_failed"))?;
    ...
    async fn cleanup_finished(&self) -> AppResult<()> {
        ...
        for status in completed {
            let exit = status.exit_code.map_or_else(|| "unknown".into(), |code| code.to_string());
            let _ = crate::logging::append(&format!(
                "[MINECRAFT EXIT] launchId={} profileId={} state={:?} exitCode={exit}",
                status.launch_id, status.profile_id, status.state,
            ));
...
fn write_launch_diagnostics(
    ...
    writeln!(file, "\n[SNine Launcher start]")?;
    writeln!(file, "Launch ID: {launch_id}")?;
    writeln!(file, "Profile ID: {profile_id}")?;
    writeln!(file, "Java: {}", java_executable.display())?;
    writeln!(file, "Game directory: {}", game_directory.display())?;
