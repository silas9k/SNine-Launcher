...
impl Phase6ContentService {
    ...
    pub async fn project(
        ...
        let versions = match self.read_active_documents(&profile) {
            Ok((manifest, _)) => {
                self.compatible_versions(project_id, kind, &manifest)
                    .await?
            }
            Err(_) => {
                // Browsing a project is read-only. A freshly created profile has
                // no V2 runtime revision yet, but that must not turn Modrinth
                // into an unusable empty screen. Installation still requires a
                // verified configured runtime in install_modrinth.
                self.modrinth
                    .project_versions(
                        project_id,
                        &VersionQuery {
