...
pub async fn phase5_launch_profile(
    ...
    let (mods_dir, _, _) = crate::snine_client_delivery::target_paths(core.inner(), &profile_id)
        .map_err(snine_launch_error)?;
    crate::snine_client_delivery::remove_legacy_s9lab_client(&mods_dir)
        .map_err(snine_launch_error)?;
    ...
    let verified_client_version =
        crate::snine_client_delivery::verify_client_ready(core.inner(), &profile_id)
            .map_err(snine_launch_error)?;
    crate::snine_client_delivery::ensure_required_support_mods(&app, core.inner(), &profile_id)
        .await
        .map_err(snine_launch_error)?;

    // Final fail-closed guard: if anything changed the profile while the update was
    // being prepared, do not launch a non-SNine runtime.
    let final_status = runtime.status(&profile_id).await.map_err(IpcError::from)?;
    let final_runtime_ok = final_status.runtime.as_ref().is_some_and(|intent| {
        intent.minecraft_version == SNINE_CLIENT_MINECRAFT_VERSION
            && intent.loader.kind == crate::runtime::LoaderKind::Fabric
            && intent
                .loader
    ...
    if final_status.install_state != "installed" || !final_runtime_ok {
        return Err(IpcError::from(AppError::coded("snine_runtime_not_ready")));
    }

    let loader_version = final_status
        .runtime
        .as_ref()
