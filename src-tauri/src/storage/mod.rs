...
impl Storage {
    ...
    pub fn rename_profile(&self, profile_id: &str, display_name: &str) -> AppResult<()> {
        let connection = self.open()?;
        connection.transaction(|transaction| {
            let changed = transaction.execute(
                "UPDATE profile_metadata SET display_name = ?2 WHERE profile_id = ?1",
                &[Value::from(profile_id), Value::from(display_name)],
            )?;
            if changed != 1 {
                return Err(AppError::coded_with(
                    "profile_not_found",
