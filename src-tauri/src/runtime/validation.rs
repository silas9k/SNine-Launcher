...
// Minecraft 26.2 and newer official releases require Java 25.  Keep this
// explicit allow-list so an intent can never request an arbitrary executable
// version, while still supporting the current Mojang runtime requirement.
const SUPPORTED_JAVA_MAJORS: [u16; 3] = [17, 21, 25];
...
mod tests {
    ...
    fn vanilla_rejects_loader_version_and_java_is_controlled() {
        ...
        intent.java = JavaPolicy::System { major_version: 25 };
        validate_profile_runtime_intent(&intent)
            .expect("the current Mojang Java 25 runtime requirement is supported");
    }
