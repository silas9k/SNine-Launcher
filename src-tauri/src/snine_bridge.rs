...
fn is_minecraft_texture_url(value: &str) -> bool {
    value.starts_with("https://textures.minecraft.net/") || value.starts_with("http://textures.minecraft.net/")
}
...
async fn download_skin_snapshot(
    ...
    if !is_minecraft_texture_url(skin_url) {
    ...
    let bytes = match fetch_skin_png(http, skin_url).await {
        Ok(bytes) => bytes,
        Err(primary_error) => {
...
pub async fn snine_launcher_live_state(
    ...
    if token.is_empty() || token.len() > 1024 || token.chars().any(char::is_whitespace) {
        return Err("snine_session_invalid".into());
    }
    let uuid = dashed_uuid(&player_uuid)?;
    let base = backend_base_url();
    let http = client()?;
    let response = http
        .get(format!("{base}/profile/{uuid}"))
        .header("X-SNine-Session", token)
        .send().await
...
pub async fn snine_launcher_cosmetics(
    ...
    let snapshot = LauncherCosmeticSnapshot {
        ok: true, player_name, online, equipped: assets, source: source.into(),
